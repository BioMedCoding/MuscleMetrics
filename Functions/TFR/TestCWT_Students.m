...
perc_discard=5;
fsamp=2048;sd=28/fsamp/10;Nlevels=500;
% Carico un segnale EMG SD simulato e ne prendo una porzione
load('EMG_4WT');sig=SD(2,2*fsamp+1:3*fsamp);
...
figure;vt=length(t)/2+1-20:length(t)/2+20;plot(t(vt),f(t(vt)));title('Mother wavelet');axis off
...
hold on;plot(t,sig);legend('original','filtered');xlabel('Time (s)');title('Signal to be processed')
...
figure;subplot(2,1,1);plot(t,sig);title('Signal');xlabel('Time (s)');
subplot(2,1,2);image((flipud(scalo')-min(scalo(:)))/range(scalo(:))*256)
colormap(gray(255));title('CWT');xlabel('Time samples');ylabel('Scales');
...
plot(y,'b');hold on;plot(sig,'r');title(['Signal and reconstruction - error = ' num2str(round(std(x(:)-sig(:))/std(sig)*10000)/100) '%']);
